	...
	.defines(function () {
		MyGame = ig.Game.extend({
			...
			init: function () {
				...
				ig.game.spawnEntity('EntityPowerup', 500, 800);
			...
			update: function () {
				...
				if (this.lastYPos - newYPos > this.blockSpawnRate - 400) {
					...
					// ig.game.spawnEntity('EntityPowerup', blockXPos, this.lastYardYPos - blocks[0].size.y);

					if (Math.floor(Math.random() * 15) == 0) {
						let newPowerup = ig.game.spawnEntity('EntityPowerup', blockXPos, this.lastYPos - blocks[0].size.y * 2);
						blocks[blocks.length].relatedEntity = newPowerup;
					}
