ig.module(
	...
	.defines(function () {
		...
		MyGame = ig.Game.extend({
			...
			portalSound: new ig.Sound('media/sounds/portal.mp3'),
			portalSoundReverse: new ig.Sound('media/sounds/portalReverse.mp3'),
			falling: new ig.Sound('media/sounds/falling.mp3'),
			slap: new ig.Sound('media/sounds/slap.mp3'),
				...
				if (!this.portalMode) {
					if (player.pos.y < 0) {
						...
						if (blocks.length > 0) {
							...
							if (player.pos.y > lowestBlock.pos.y && !this.playerDead) {
								let playerPosBeforeDead = player.pos.y;
								player.vel.y = 2000;
								this.playerDead = true;
								score = -this.lastYPos;
								console.log("playing fallign");
								this.falling.play();
								console.log("Falling")
								console.log(score);
							}
					...
					if (portals[i].collidingWithPlayer) {
						...
						setTimeout(() => {
							...
							for (let i = 0; i < portalBlocks.length; i++) {
								...
								console.log("this.portalSoundReverse.play();");
							}
							this.portalSoundReverse.play();
						}, 5000)
			...
			update: function () {
				...
				if (football) {
					if ((xmin <= (football.pos.x + 0.5 * football.size.x) && (football.pos.x - 0.5 * football.size.x) <= xmax) && ymin <= football.pos.y && football.pos.y <= ymax) {
						...
						if (whichSound == 0) {
							console.log("s1");
							soundToPlay = "cheering1";
						} else {
							console.log("s2");
							soundToPlay = "cheering2";
						}
		...
		EnterHighscore = ig.Game.extend({
			...
			cheering1: new ig.Sound('media/sounds/cheering_1.mp3'),
			cheering2: new ig.Sound('media/sounds/cheering_2.mp3'),
			...
			update: function () {
				...
				if (soundToPlay == "cheering1") {
					this.cheering1.play();
					soundToPlay = null;
				} else if (soundToPlay == "cheering2") {
					this.cheering2.play();
					soundToPlay = null;
				}
		...
		Highscore = ig.Game.extend({
			...
			click: new ig.Sound('media/sounds/click.mp3'),
			select: new ig.Sound('media/sounds/select.mp3'),
		...
		FieldgoalMenu = ig.Game.extend({
			...
			looser: new ig.Sound('media/sounds/looser.mp3'),
